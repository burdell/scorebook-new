...
export const appRouter = router({
  getScorecards: publicProcedure.query(() => {
    return mockScorecards
  }),

  getScorecardById: publicProcedure
    .input(z.object({ id: z.string() }))
    .query(({ input }) => {
      return mockScorecards.find((sc) => sc.id === input.id) || null
    }),

  getScorecardsByDate: publicProcedure
    .input(z.object({ date: z.string() }))
    .query(({ input }) => {
      return mockScorecards.filter((sc) => sc.gameDate === input.date)
    }),
})
